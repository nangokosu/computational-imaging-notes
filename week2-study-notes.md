...
**Diagram.** Picture the lens as a single point on the optical axis, with the sensor a fixed distance *f* behind it (the same right-triangle picture as Week 1 §9's Fig. 9, but solving for the opposite unknown): a ray from the lens to the top edge of the sensor and one to the bottom edge together span the full field of view; the dashed optical axis bisects that angle into two equal halves of FOV/2, each one the angle of a right triangle with opposite leg *d*/2 (half the sensor dimension) and adjacent leg *f* — exactly the triangle `arctan(d/(2f))` measures, doubled back into the full angle. See the artifact for the full drawn version.
...
### 9.2.1 Two ray pictures, one formula: why near-defocus and far-defocus give the same-sized blur

For the actual object at *O*, its own rays converge to a genuine focus point at some distance from the lens — call it *S′_O*, found the same way §4.1 finds any image distance, by plugging *O* (not *S*) into the thin lens equation: 1/*O* + 1/*S′_O* = 1/*f*. This is generally *not* where the sensor sits: the sensor sits at *S′*, the distance fixed by wherever the lens is actually focused (*S*, §9.1). Only when *O* = *S* does *S′_O* land exactly on the sensor — a sharp point. Whenever *O* ≠ *S*, *S′_O* and *S′* disagree, and *which side* of the sensor *S′_O* falls on is what produces two physically different-looking pictures of blur:

- **Far side (*O* > *S*).** A farther object forms its own image *closer* to the lens (§4.1's inverse relationship between object and image distance: bigger *O* means smaller *S′_O*), so *S′_O* < *S′* — the actual object's rays cross to a point *before* the sensor, then keep traveling and have already started re-diverging by the time they reach the fixed sensor plane. The blur disc is a cross-section of an **already-diverged** cone. (This is the picture already drawn in §9.2's circle-of-confusion construction.)
- **Near side (*O* < *S*).** A closer object forms its own image *farther* from the lens (smaller *O* means bigger *S′_O*), so *S′_O* > *S′* — the actual object's rays are still narrowing, heading toward a crossing point *beyond* the sensor, when the sensor intercepts them first. The blur disc is a cross-section of a **not-yet-converged** cone. (This is the picture already drawn in §5's defocus figure.)

**Why both give the exact same formula.** Picture the full ray bundle from any actual object as an hourglass (a **bicone**): it narrows from the full aperture down to a single point — the object's own focus point *S′_O* — then widens again, symmetrically, on the far side of that point. This is a plain fact about straight lines converging to a point: the diverging half is a mirror image of the converging half, at the same opening angle, because the two halves are literally the same rays, extended straight through their common crossing point. The sensor is nothing more than a fixed cross-sectional slice through this bicone, always taken at the same location (*S′* from the lens), while the bicone's own waist (*S′_O*) moves depending on which actual object is being imaged. Sliced at an axial distance *Δ* from the waist, a cone's cross-section has a width set only by |*Δ*| and the cone's fixed opening angle (itself set by the aperture *D* and the waist distance) — the geometry has no way to "know" whether the slice was taken before or after the waist; it only depends on how far away the slice is. That is the physical reason §9.2's formula depends only on |*O* − *S*|, never on the *sign* of *O* − *S*: the converging (near-object) and diverging (far-object) cases are mirror images of the exact same cone geometry, just sliced on opposite sides of the same waist.

**Applied to the near and far boundaries (§9.3.1).** *O_near* and *O_far* are two completely different physical scenarios — one a still-narrowing cone, the other an already-diverged one — belonging to two different actual objects, at two different distances from *S*. Nothing requires these to look alike physically, and they don't: one is caught before it has finished converging, the other after it has already crossed over and started to spread. But because both are governed by the identical |*O* − *S*| relationship, choosing the same acceptable-blur threshold *ε* on both sides necessarily lands on the *same* disc diameter *ε* at both boundaries — not because the two ray pictures are physically alike, but because the equation relating a cone's cross-section to its distance from its own waist doesn't care which side of the waist that distance is measured on.
