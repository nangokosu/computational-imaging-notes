...
*(Reconstruction note: the lecture's own slide for this matrix survived PDF extraction with its exact numeric coefficients garbled. The values above are the standard BT.601 RGB↔Y′CbCr matrix — the same one the slide's own structure, offsets of 16/128/128 and a matrix-inverse relationship, points to — reconstructed from the known standard rather than read cleanly off the slide. Flagged here for a spot-check.)*
...
**PS2's practical framing.** Although there are several interpolation cases above, by symmetry there are only **4 unique filter shapes** needed in total (several cases are the same filter rotated or with color roles swapped). PS2 also flags a genuinely useful implementation detail: many of the filter coefficients that fall out of this derivation are exact powers of two — which, in fixed-point hardware, means a multiplication can be implemented as a cheap bit-shift instead of a general multiply, a real reason this specific linear filter design became popular in actual camera ISPs, not just an academic curiosity.
