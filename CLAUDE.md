...
- For every critical formula behind a key concept, fully explain it, not just state it: (1) what each component/variable stands for — what it physically means, its units, and whether it's something the reader controls or something fixed by the setup; (2) the derivation intuition — why the formula takes that specific shape, walked through step by step, not just asserted as a result; and (3) whenever the formula is geometric in nature (built from angles, distances, ray paths, or similar spatial relationships), a diagram showing exactly the quantities the formula relates, per the diagram rule below — the diagram and the derivation should visibly correspond, so the reader can point at a line in the figure for every symbol in the formula.
...
- **When a subsection introduces a stricter sub-term for a word already used loosely elsewhere** (e.g. "image frequency," cycles/pixel, as a precise sub-flavor of the broader "spatial frequency" umbrella), audit every other place that word is used in that same technical context — earlier sections and later ones, both files, and the artifact — not just the subsection being written. Either switch each use to the precise term or leave a short note explaining why the looser term is still correct there. Overloading doesn't only appear at first introduction; a document that later splits a word into precise sub-terms can silently drift back into the loose one anywhere it's reused, and that drift is exactly as confusing as never disambiguating at all.
...
- Avoid dense, unbroken blocks of text — a long paragraph signals a concept needs restructuring, not just tighter prose. Split multi-idea paragraphs at idea boundaries, pull worked examples/asides into their own callout blocks, use lists for enumerations, and add a diagram wherever one would carry weight prose is carrying alone. This matters most in first-principles sections built per the homework-depth rule above, where density accumulates fastest.
- No historical asides. Skip the history of a technique, instrument, or field, and skip biographical background on the people associated with a name (e.g. who a transform or law is named after, when or why it was discovered). Stay on the technical content the term/concept needs to be understood and used — a person's or paper's name is mentioned only when it's how the concept is actually referred to in the course, never as a launching point for a historical narrative.
...
Diagrams live only in the artifact, never in the markdown files. For any geometric or quantitative concept worth illustrating, first check whether the lecture slide deck already has a diagram/photo for it: if so, crop that exact region from the rendered slide image (see "Source material" above) and publish it as a supporting file on the artifact, embedded via `<figure class="chart-fig"><img src="..." ...></figure>`, rather than redrawing it — reuse the source image instead of authoring a new one. Only build an original inline SVG `<figure class="chart-fig">` (styled with the existing CSS custom properties — `var(--accent)`, `var(--accent-2)`, `var(--accent-3)`, `var(--border)`, `var(--ink-faint)`, `.chart-label` — so it stays theme-aware) when no suitable slide image exists, e.g. a concept the slides don't diagram, or a comparison/composite view the deck never shows in one place. Every figure, slide-sourced or original, gets a caption crediting what it shows plainly (no separate "adapted from" history needed). Figure captions are numbered "Fig. N" sequentially in document order across the whole page (not per week), so inserting a new figure ahead of existing ones means renumbering the ones after it. The `weekN-study-notes.md` files stay plain text/tables only, matching the pattern set by Week 1.
...
2. Write `weekN-study-notes.md` following the Week 1 structure and tone, arrived at through this sequence rather than by drafting prose directly off the slides:
   a. **Identify all concepts the lecture actually depends on**, prioritizing anything a current or upcoming homework leans on for the deepest treatment (see the homework-depth rule above), even if the lecture itself only mentions them in passing.
...
4. Update the running artifact (URL above) by reading it first (`action: "read"`), then republishing with `url` set to that same address: append a new `.week-block` after the previous one (before the `<hr class="div"/>` that precedes `#glossary`), and refresh the matching `#glossary` `.week-card` so it reflects what was actually covered rather than a forward-looking guess. Reuse the established visual language exactly (Source Serif 4 / Public Sans / IBM Plex Mono, the warm-amber-on-indigo-slate palette) — don't redesign per week. For figures, pull cropped slide images from the deck wherever the deck already diagrams the concept (see "Published artifact" above), falling back to an original inline SVG chart only when the deck has nothing to reuse. Wikipedia-link every newly introduced term the same way as Week 1 (see "Hyperlinks" above). Keep the nav script's coding standard (JSDoc per function, fully descriptive names, no unexplained hardcoded numbers). Load the `artifact-design` skill before touching the HTML.
