...
1. **Intuition** — why the formula takes that specific shape, walked through step by step (the derivation intuition), not just asserted as a result.
...
3. **Diagram, if geometric** — whenever the formula is geometric in nature (built from angles, distances, ray paths, or similar spatial relationships), an illustrative diagram showing exactly the quantities the formula relates, per the diagram rule in "Published artifact" below. The diagram and the derivation should visibly correspond, so the reader can point at a line in the figure for every symbol in the formula.
