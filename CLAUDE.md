...
- Wherever a concept is secretly a linear-algebra operation, name it explicitly and explain it from scratch in that context: e.g. converting between color spaces (RGB → XYZ → sRGB) is a **change of basis** by a 3×3 matrix; metamerism is a projection with a non-trivial null space; a gamut is a convex hull of primary vectors; a linear filter is a matrix acting on the image vector; a matrix inverse undoes a transform. State which vectors, basis, and matrix are involved, so the reader can connect the imaging idea to the math they already know. **Every one of these "Linear-algebra view" callouts must be paired with a matrix visualization in the artifact, never left as prose alone** — see the matrix-diagram construction rules under "Published artifact" below.
...
- `formulas.md` — the single, cumulative formula reference, grouped by the week each formula is introduced in (mirroring `glossary.md`'s per-week structure and its no-hyperlinks convention). One entry per formula: the formula exactly as written in that week's notes, a term-by-term table (symbol, meaning, units where relevant, controlled-by-you vs. fixed-by-setup), and a one-sentence "Computes" line stating what real-world or computational quantity it produces. Covers only formulas load-bearing enough to get the full "Equations" treatment in that week's notes, not every one-off numeric substitution in a worked example. Update it every time a new week's notes are written: append that week's formulas as a new `## Week N` section, in the order they appear in the notes. Never delete or renumber an existing week's entries.
...
**Artifact updates require explicit invocation.** Editing the markdown notes (`weekN-study-notes.md`, `glossary.md`, `formulas.md`) and committing/pushing them is automatic per this project's other rules and needs no separate go-ahead. Updating the published artifact is not automatic in the same way: never dispatch an artifact-editing subagent, or edit the artifact directly, as a side effect of a markdown-only content fix — adding a formula, clarifying a definition, highlighting a passage, deepening an existing section, and similar incremental edits — unless Khai explicitly asks for the artifact to be updated too. The one exception is the `add-lecture-week` workflow: adding a whole new week is a single requested operation that inherently includes updating the artifact, since that's the only place a new week's diagrams live.
...
**Matrix diagrams:** every "Linear-algebra view" callout gets its own `<figure class="chart-fig">` immediately following it, drawn to the same construction rules as any other diagram (theme-aware CSS custom properties, `.chart-label`, numbered "Fig. N" caption) but built specifically to make the matrix or vector operation visible rather than just named:

- Draw the actual matrix as a grid of labeled cells (rows × columns sized and captioned to the real dimensions in play, e.g. 3×N or P×3P) — shade or color-code entries to show structure at a glance: which entries are zero vs. nonzero, a diagonal, a single 1 per row (selection matrix), or a repeating block pattern.
- Show the operation as a flow: input vector → matrix → output vector, each drawn as a labeled column/row of cells, with an arrow or bracket connecting them, so the reader can see the matrix–vector product happening, not just read the equation.
- For a **null space**, sketch the geometric picture — a line or plane through the origin inside a small labeled coordinate system, distinguishing it from the row space / range.
- For a **change of basis**, draw both bases (old and new axes or primary vectors) in the same coordinate sketch with the transformed vector shown in each.
- For a **convex hull** (e.g. a gamut), draw the filled polygon with its defining vertex vectors labeled and an example point inside vs. outside it.
- For a **projection**, show the original vector, its projection, and the residual/null-space component as distinct labeled arrows.
